...
public class Bucket implements Runnable
{
	...
	private void updateNoTokens()
	{
		...
		long addTokens = timeElapsed/tokenInterval;

		if (noTokens + (int)addTokens >= size)
			noTokens = size;
		else
			noTokens += (int)addTokens;

		//subtract the modulus to ensure that halfway-generated tokens are preserved.
		lastTime = thisTime-(timeElapsed%tokenInterval);
	}
	...
	public synchronized long getWaitingTime(int tokensToWaitFor)
	{
		...
		long tokensRequired = tokensToWaitFor-noTokens;
		return (tokensRequired * tokenInterval);
	}
}
