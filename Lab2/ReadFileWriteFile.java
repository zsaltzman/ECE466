...
class ReadFileWriteFile {  
	public static void main (String[] args) { 
		...
		int itotal, ptotal, btotal, inum, pnum, bnum;
		inum = pnum = bnum = itotal = ptotal = btotal = 0;
		...
		try {  
			...
			while ( (currentLine = bis.readLine()) != null) { 
				...
				if (Ftype.equals("I")) {
					...
					System.out.println("Input not as expected, cannot calculate size for this packet.");
				}
			...
			pout.println("Average b frame: " + btotal/bnum + "\t" + "Average p frame: " + ptotal/pnum + "\t" + "Average i frame: "
					+ itotal/inum); 
			System.out.println("Average b frame " + btotal/bnum);
			System.out.println("Average i frame " + itotal/inum);
			System.out.println("Average p frame " + ptotal/pnum);
			// Close files   
			if (bis != null) { 
				try { 
					bis.close(); 
