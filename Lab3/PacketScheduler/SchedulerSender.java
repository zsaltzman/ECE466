...
{	
	...
	public void run()
	{
		//average packet size assumed to be 100 bytes
		int averagePacketSize = 100;
		...
		//determine end-of-round allocations
		//since each stream has same average packet size, we can simplify the process
		int min = Math.min(Math.min(w1, w2), w3);
		...
		while(true)
		{
			...
			if (System.nanoTime() >= nextSendOK)
			{
				 ...
				 */
				if ((packet = buffers[0].peek()) != null && rr1 > 0) 
				{
					sendPacket(packet, startTime);
					buffers[0].removePacket();
					rr1--;
				}
				else if ((packet = buffers[1].peek()) != null && rr2 > 0) 
				{
					sendPacket(packet, startTime);
					buffers[1].removePacket();
					rr2--;
				}
				else if ((packet = buffers[2].peek()) != null && rr3 > 0)
				{
					sendPacket(packet, startTime);
					buffers[2].removePacket();
					rr3--;
				}
				else
				{
					...
					//give each queue its allocation after round is over.
					rr1 = a1;
					rr2 = a2;
					rr3 = a3;
				}
			...
			{
				//NOTE: Replaced calls to Thread.sleep() with busy wait
				//Thread.sleep not reliable for such low time granularities!
				// wait until it is possible to send
				long timeToWait = nextSendOK-startTime;
				long bTime = System.nanoTime();
				long timeWaited = bTime;
				while(timeWaited-bTime < timeToWait) {
